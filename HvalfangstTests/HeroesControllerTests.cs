...
namespace HvalfangstTests
{
    public class HeroesControllerTests
    {
        ...
        public async Task Post_ShouldCreateHero_AndReturnOk(string name, string heroClass, int level, int hitPoints, int damage, int attack, int armorClass)
        {
            // Arrange
        ...
        public async Task Post_ShouldFailValidation_AndReturnBadRequest(string name, string heroClass, int level, int hitPoints, int damage, int attack, int armorClass)
        {
            // Arrange
            var heroService = Substitute.For<HeroService>(Substitute.For<IHeroRepository>());
            var logger = Substitute.For<ILogger<HeroesController>>();
